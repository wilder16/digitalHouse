...
const templateLiteral = (noticias) => {
    return `
        <h2>${noticias.titulo}</h2>
        <img src="${noticias.imgUrl}" alt="">
        <div>${noticias.descripcion}</div>
        <div>${noticias.fecha}</div>
        <img src= ${noticias.tipoNacional ? "img/colombia.png" : "img/internacional.png"}></div>
    `
}

const nodoPadre = document.querySelector("section");
console.log(nodoPadre);

noticias.forEach( (noticia) => nodoPadre.innerHTML+=templateLiteral(noticia) );
